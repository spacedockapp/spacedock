    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT < 19) {
            return new AlertDialog.Builder(getActivity())
                    ...
                    .setCancelable(true)
                    .setMessage(
                            "Printing is not available on this version of Android.\nPlease update to 4.4 or newer.")
            ...
            builder.setView(inflater.inflate(R.layout.printfleet_dialog, null))
                    .setPositiveButton(R.string.print, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int id) {
                            String name = ((EditText) getDialog().findViewById(R.id.name_input))
                                    .getText().toString();
                            String email = ((EditText) getDialog().findViewById(R.id.email_input))
                                    .getText().toString();
                            String faction = ((EditText) getDialog().findViewById(
                                    R.id.faction_input)).getText().toString();
                            String event = ((EditText) getDialog().findViewById(R.id.event_input))
                                    .getText().toString();
                            DatePicker date = (DatePicker) getDialog()
                                    .findViewById(R.id.event_date);
                            String dates = String.format("%02d-%02d-%04d", date.getMonth() + 1,
                                    date.getDayOfMonth(), date.getYear());
                            Boolean blindbuy = ((ToggleButton) getDialog().findViewById(
                                    R.id.blind_buys_toggle)).isChecked();
                            Squad squad = Universe.getUniverse().getSquadByUUID(mSquadUuid);
                            PrintManager printmanager = (PrintManager) getActivity()
                                    .getSystemService(Context.PRINT_SERVICE);
                            String jobname = squad.getName();

                            PrintAttributes attribs = new PrintAttributes.Builder().setMediaSize(
                                    PrintAttributes.MediaSize.NA_LETTER.asPortrait()).build();

                            printmanager.print(jobname, new PrintFleetAdapter(null, squad, name,
                                    email, faction, event, dates, blindbuy), attribs);
                        }
                    })
